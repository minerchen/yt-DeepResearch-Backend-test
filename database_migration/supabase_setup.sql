...
-- Research Sessions Table (for both individual runs and comparison sessions)
...
-- Research Results Table (for all model runs, both individual and comparison)
...
-- DEPRECATED: Keep for backward compatibility, but new code should use research_sessions
...
-- DEPRECATED: Keep for backward compatibility, but new code should use research_results
...
CREATE POLICY "Allow all operations on research_sessions" ON research_sessions
    FOR ALL USING (true);

CREATE POLICY "Allow all operations on research_results" ON research_results
    FOR ALL USING (true);
...
-- Analytics views using the new unified structure
...
-- View for individual research sessions (Research tab)
...
-- View for comparison sessions (Compare tab)
...
CREATE OR REPLACE VIEW legacy_model_performance_summary AS
SELECT 
    model,
    COUNT(*) as total_requests,
    ROUND(AVG(duration)::NUMERIC, 2) as average_duration,
